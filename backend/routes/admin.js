...
router.get('/stats', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const leavingSlots = await ParkingSlot.countDocuments({ status: 'leaving' });
    ...
    const pendingOccupiedRequests = await ParkingSlot.countDocuments({ occupiedRequestStatus: 'pending' });
    const pendingLeavingRequests = await ParkingSlot.countDocuments({ leavingRequestStatus: 'pending', paymentStatus: 'paid' });
...
router.patch('/slots/:slotId/status', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const updatedSlot = await ParkingSlot.findById(slot._id).populate('bookedBy', 'name email');
...
router.post('/slots/:slotId/release', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { slotId } = req.params;
...
// Block a user
router.post('/users/:userId/block', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { userId } = req.params;
...
// Unblock a user
router.post('/users/:userId/unblock', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { userId } = req.params;
...
router.get('/requests', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (type === 'occupied') {
      ...
      query = { leavingRequestStatus: 'pending', paymentStatus: 'paid' };
    } else {
      query = {
        $or: [
          { occupiedRequestStatus: 'pending' },
          { leavingRequestStatus: 'pending', paymentStatus: 'paid' }
        ]
      };
    }
...
router.post('/approve-occupied/:slotId', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { slotId } = req.params;
    const slot = await ParkingSlot.findOne({ id: slotId });
    ...
    if (slot.occupiedRequestStatus !== 'pending') {
      return res.status(400).json({ error: 'No pending occupied request found' });
    }

    // Generate occupied QR code
    const occupiedQrData = JSON.stringify({
      slotId: slot.id,
      userId: slot.bookedBy.toString(),
...
router.post('/reject-occupied/:slotId', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { slotId } = req.params;
    const slot = await ParkingSlot.findOne({ id: slotId });
...
router.post('/approve-leaving/:slotId', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { slotId } = req.params;
    const adminId = req.user.id;
    ...
    if (slot.leavingRequestStatus !== 'pending') {
      return res.status(400).json({ error: 'No pending leaving request found' });
    }
...
router.get('/completed-parkings', authenticateToken, requireAdmin, async (req, res) => {
  try {
